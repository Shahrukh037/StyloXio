...
let PRODUCTS = []; // Dynamically loaded products
...
function changeQty(idx,delta){ const cart=getCart(); if(!cart[idx]) return; cart[idx].qty+=delta; if(cart[idx].qty<=0) cart.splice(idx,1); saveCart(cart); renderCartPage(); }
function removeItem(idx){ const cart=getCart(); if(!cart[idx]) return; cart.splice(idx,1); saveCart(cart); renderCartPage(); showToast('Item removed'); }
...
function renderCheckoutPreview(){
  ...
  cart.forEach(i=>{
    ...
    d.innerHTML=`<div style="display:flex;justify-content:space-between"><div>${escapeHtml(i.name)} x ${i.qty}</div><div>₹${i.price*i.qty}</div></div>`;
  ...
  orders.slice().reverse().forEach(o=>{
    ...
    div.innerHTML=`
      <div style="display:flex;justify-content:space-between">
        ...
        <div style="text-align:right"><strong>Items:</strong><br>${o.items.map(i=>`${escapeHtml(i.name)} x ${i.qty}`).join('<br>')}</div>
      </div>
    `;
    container.appendChild(div);
  });
...
document.addEventListener('input',function(e){
  if(e.target?.id==='searchInput') renderProductsPage();
  if(e.target?.id==='filterCategory') renderProductsPage();
});

/* ================== CART COUNT STORAGE SYNC ================== */
